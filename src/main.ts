    ...
    if (!this.config.TeslaFiAPIToken) {
      ...
      try {
        ...
        if (await teslaFiAPICaller.ReadTeslaFi()) {
          ...
          this.log.warn(
            `Got no data from TeslaFi - adapter restarts in 5 minutes`,
          );
        ...
        this.log.error(
          teslaFiAPICaller.generateErrorMessage(
            error,
            `pull of data from TeslaFi-Server`,
          ),
        );
      }
      ...
      const jobVehicleData = setInterval(
        async () => {
          this.log.debug(
            `Interval job VehicleData - Result: ${await teslaFiAPICaller.ReadTeslaFi()}`,
          );
        },

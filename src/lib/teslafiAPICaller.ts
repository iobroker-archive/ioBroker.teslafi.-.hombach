...
function convertUnixToLocalTime(
...
function calculateEndTimeFromNow(
  hours: number,
  dateFormat = "dd.MM.yyyy HH:mm:ss",
): string {
  ...
  async ReadTeslaFi(): Promise<boolean> {
    try {
      const response = await axiosInstance.get(
        `${this.queryUrl}${this.adapter.config.TeslaFiAPIToken}&command=`,
        {
      ...
      if (result.response?.result === "unauthorized") {
        this.adapter.log.warn(
          `TeslaFI data read - unauthorized access detected - please verify your API Token`,
        );
        return false;
      }
      ...
      void this.checkAndSetValue(
        `vehicle-data.rawJSON`,
        response.data,
        `JSON raw data from TeslaFi`,
      );
      ...
      if (stVD.display_name.value !== null) {
        //"Red Elephant"
        void this.checkAndSetValue(
          `vehicle-data.${stVD.display_name.key}`,
          stVD.display_name.value,
          stVD.display_name.desc,
        );
      }
      //#endregion
      ...
      if (stVD.carState.value !== null) {
        //"Idling"
        void this.checkAndSetValue(
          `vehicle-state.${stVD.carState.key}`,
          stVD.carState.value,
          stVD.carState.desc,
        );
      }
      ...
      if (stVD.charging_state.value !== null) {
        ...
        void this.checkAndSetValue(
          `vehicle-state.${stVD.charging_state.key}`,
          stVD.charging_state.value,
          stVD.charging_state.desc,
        );
      } else {
        void this.checkAndSetValue(
          `vehicle-state.${stVD.charging_state.key}`,
      ...
      if (stVD.car_version.value !== null) {
        ...
        void this.checkAndSetValue(
          `vehicle-state.${stVD.car_version.key}`,
          stVD.car_version.value,
          stVD.car_version.desc,
        );
      }
      if (stVD.newVersion.value !== null) {
        //" "
        void this.checkAndSetValue(
          `vehicle-state.${stVD.newVersion.key}`,
          stVD.newVersion.value,
          stVD.newVersion.desc,
        );
      } else {
        void this.checkAndSetValue(
          `vehicle-state.${stVD.newVersion.key}`,
      ...
      if (stVD.location.value !== null) {
        //"Home"
        void this.checkAndSetValue(
          `vehicle-state.${stVD.location.key}`,
          stVD.location.value,
          stVD.location.desc,
        );
      }
      if (stVD.longitude.value !== null) {
        //"9.899749"
        void this.checkAndSetValue(
          `vehicle-state.${stVD.longitude.key}`,
          stVD.longitude.value,
          stVD.longitude.desc,
        );
      }
      if (stVD.latitude.value !== null) {
        //"49.873095"
        void this.checkAndSetValue(
          `vehicle-state.${stVD.latitude.key}`,
          stVD.latitude.value,
          stVD.latitude.desc,
        );
      }
      ...
      if (stVD.charger_phases.value !== null) {
        //"3"
        void this.checkAndSetValueNumber(
          `battery-state.${stVD.charger_phases.key}`,
          parseFloat(stVD.charger_phases.value),
          stVD.charger_phases.desc,
        );
      } else {
        void this.checkAndSetValueNumber(
          `battery-state.${stVD.charger_phases.key}`,
      ...
      if (stVD.scheduled_charging_start_time.value !== null) {
        ...
        void this.checkAndSetValue(
          ...
          convertUnixToLocalTime(
            parseFloat(stVD.scheduled_charging_start_time.value),
          ),
  ...
  private async HandleConnectionError(
    ...
    if (stError.response) {
      ...
      switch (stError.response.status) {
        case 401:
          ...
          this.adapter.log.error(
            `The TeslaFi API request has not been completed because it lacks valid authentication credentials.`,
          );
          this.adapter.log.error(
            `HTTP error 401 when calling ${sOccasion}!! (e${sErrorOccInt}.0)`,
          );
        ...
        default:
          this.adapter.log.error(
            `HTTP error ${stError.response.status} when polling ${sOccasion}!! (e${sErrorOccInt}.1)`,
          );
      }
      ...
      switch (stError.code) {
        case "ETIMEDOUT":
          this.adapter.log.warn(
            `Connection timeout error when calling ${sOccasion}`,
          );
          this.adapter.log.warn(
            `Please verify the API Token or adapt your poll interval, (e${sErrorOccInt}.2)`,
          );
          break;
        case "EHOSTUNREACH":
          this.adapter.log.warn(
            `TeslaFi not reachable error when calling ${sOccasion}`,
          );
          this.adapter.log.warn(
            `Please verify your network environment, (e${sErrorOccInt}.2)`,
          );
          break;
        case "ENETUNREACH":
          this.adapter.log.warn(
            `Inverter network not reachable error when calling ${sOccasion}`,
          );
          this.adapter.log.warn(
            `Please verify your network environment, (e${sErrorOccInt}.2)`,
          );
      ...
      this.adapter.log.error(
        `Unknown error when calling ${sOccasion}: ${stError.message}`,
      );
      this.adapter.log.error(
        `Please verify the API Token or adapt your poll interval, (e${sErrorOccInt}.3)`,
      );
      ...
      if (
        ...
        if (sentryInstance) {
          const oldError = await this.adapter.getStateAsync(
            "LastSentryLoggedError",
          );
            ...
            Sentry &&
              Sentry.withScope(
                (scope: {
                  ...
                  setTag: (arg0: string, arg1: number | string) => void;
                }) => {
                  scope.setLevel("info");
                  scope.setTag("Hour of event", date.getHours());
                  Sentry.captureMessage(
                    `Catched error: ${stError.message}`,
                    "info",
                  );
                },
              );

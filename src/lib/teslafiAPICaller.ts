...
export class TeslaFiAPICaller extends ProjectUtils {
		...
		try {
			...
			if (stVD.time_to_full_charge.value !== null) {
				...
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.time_to_full_charge.key}`,
			...
			if (stVD.charge_current_request.value !== null) {
				//"16"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.charge_current_request.key}`,
			...
			if (stVD.charge_limit_soc.value !== null) {
				//"80"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.charge_limit_soc.key}`,
			...
			if (stVD.usable_battery_level.value !== null) {
				//"75"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.usable_battery_level.key}`,
			...
			if (stVD.est_battery_range.value !== null) {
				//"208.25"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.est_battery_range.key}`,
					parseFloat(stVD.est_battery_range.value),
					stVD.est_battery_range.desc,
					"mi",
				);
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.est_battery_range.key}_km`,
			...
			if (stVD.seat_heater_right.value !== null) {
				//"0"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.seat_heater_right.key}`,
			...
			if (stVD.seat_heater_rear_left.value !== null) {
				//"2"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.seat_heater_rear_left.key}`,
			...
			if (stVD.seat_heater_rear_center.value !== null) {
				//"0"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.seat_heater_rear_center.key}`,
			...
			if (stVD.seat_heater_rear_right.value !== null) {
				//"3"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.seat_heater_rear_right.key}`,
			...
			if (stVD.seat_heater_rear_left_back.value !== null) {
				//"3"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.seat_heater_rear_left_back.key}`,
			...
			if (stVD.seat_heater_rear_right_back.value !== null) {
				//"2"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.seat_heater_rear_right_back.key}`,
			...
			if (stVD.steering_wheel_heater.value !== null) {
				//"0"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.steering_wheel_heater.key}`,
			...
			if (stVD.driver_temp_setting.value !== null) {
				//"20.5"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.driver_temp_setting.key}`,
			...
			if (stVD.odometer.value !== null) {
				//"16434.079511"
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.odometer.key}`,
					Math.round(parseFloat(stVD.odometer.value) * 100) / 100,
					stVD.odometer.desc,
					"mi",
				);
				this.checkAndSetValueNumber(
					`vehicle-data.${stVD.odometer.key}_km`,
	...
	/****************************************************************************************
	 * ReadTeslaFi **************************************************************************/
	async ReadTeslaFiOLD(): Promise<boolean> {
		await axios
						...
						if (value !== null) {
							switch (key) {
								...
								case "time_to_full_charge": //"0.0"
									this.checkAndSetValueNumber(`vehicle-data.${key}`, parseFloat(value as string), `Time to full charge`);
									break;
								case "charge_current_request": //"16"
									this.checkAndSetValueNumber(
										`vehicle-data.${key}`,
										parseFloat(value as string),
										`requested charge current by your car`,
										"A",
									);
									break;
								// charge_enable_request: "1", charge_to_max_range: ""
								...
								case "outside_temp": //"14.0"
									this.checkAndSetValueNumber(
										`vehicle-data.${key}`,
										parseFloat(value as string),
										`outside temperature near your Tesla`,
										"°C",
									);
									break;
